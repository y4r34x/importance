...
Usage:
    ...
    python -m negotiation.extraction.llm_extractor seebeks.json --url https://sec.gov/...
"""
...
from .schemas import SECTION_FIELD_MAPPING, TSV_COLUMNS, FIELD_DEFINITIONS
...
def get_field_definitions(fields: list) -> str:
    ...
    return "\n".join(f"- {FIELD_DEFINITIONS[f]}" for f in fields if f in FIELD_DEFINITIONS)
...
def main():
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='data/extracted/raw.tsv',
        help='Output TSV file path (default: data/extracted/raw.tsv)'
    )
    parser.add_argument(
        '--model',
        type=str,
